...
struct fftMatchResult {
  ...
  // Normalised corss-corelation score with +1.0 being highest and -.0 being
  // inverted and 0 being no
  double Score;
};
...
void saveAudioVectorToWav(
    const std::string &filename, std::vector<float> &audio, int sampleRate,
    int channels /*Assumes default format as in CaptureAudio()*/);