...
std::string GetCurrentPlayingInfo() {
  return executeCommand(
      std::string("playerctl metadata --format '{{ artist }} - {{ title }}'"));
}
...
void runVideoLoop(stateClass &state) {
  ...
  while (running == true) {
    ...
    if (trueRes != state.resYRequested) {
      state.out("Best possible resolution was:" + std::to_string(trueRes) +
                    ". Resizing...",
                4);
      state.resX = trueRes * 16 / 9;
      state.resY = trueRes;
      state.out("SDL window size is now " + std::to_string(trueRes), 4);
    }

    state.out("Attempting to restart song...", 4);
    restartSong();

    state.out("Capturing " + std::to_string(state.CaptureTime) +
                  " seconds of audio into sample.wav",
              4);
    ...
    state.out(
        "Offset in samples:" + std::to_string(result.offsetInSamples) +
            "\n Offset in Seconds:" + std::to_string(result.offsetInSeconds) +
            "\n BestMatchScore:" + std::to_string(result.Score),
        4);
    if (result.Score < 0.5)
    ...
    SDL_Window *window =
        SDL_CreateWindow("P69", state.resX, state.resY, SDL_WINDOW_RESIZABLE);

    if (!window)
      InvalidInputMessage("Failed to create SDL window: " +
                          std::string(SDL_GetError()));

    const std::string vlcverbosearg =
        "--verbose=" + std::to_string(state.verbose);
    const char *args[] = {vlcverbosearg.c_str()
    ...
    if (!vlc) {
      if (libvlc_errmsg() != 0)
        InvalidInputMessage(
            "Failed to initialise libVLC. Libvlc error message: " +
            std::string(libvlc_errmsg()));
      else
        InvalidInputMessage("Failed to initialise vlc; there is no vlc error "
                            "message to provide");
    }
    ...
    if (!media) {
      if (libvlc_errmsg() != 0)
        InvalidInputMessage(
            "Failed to create new vlc media from:" + videoName +
            ". Libvlc error message: " + std::string(libvlc_errmsg()));
      else
        InvalidInputMessage("Failed to create new vlc media from:" + videoName +
                            "; there is no vlc error message to provide");
    }
    ...
    while (running &&
           libvlc_media_player_get_state(mediaplayer) != libvlc_Ended &&
           GetCurrentPlayingInfo() == current) {
...
int getVideoHeight(const std::string &videoName) {
  int y = stoi(executeCommand("ffprobe -v error -select_streams v:0 "
