...
int main(int argc, char *argv[]) {
  ...
  //CLI config
  CLI::App app{"P69(-linux):A Portable Music Visualiser in the form of Musicvideos and (tbd) Lyrics. COPYRIGHT 2026 MrHunor, GPLv3"};
  auto *media_group = app.add_option_group("Media Visulasation Type (Video or Lycris)");
  ...
  app.callback([&]()
  {
  if(*video) runVideoLoop(state);
  if(*lyrics) InvalidInputMessage("Sorry but Lyrics mode is yet to be implemented");
